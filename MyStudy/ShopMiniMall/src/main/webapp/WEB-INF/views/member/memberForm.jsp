...
function req() {
	...
	httpRequest.open("get", "memberIdCheck?userid="+id, true);
...
function validcheck() {
	...
	if(id.length==0){
		alert("아이디 필수")
		event.preventDefault();
	}else if(document.querySelector("#passwd").value.length==0){
		alert("비밀번호 필수")
		event.preventDefault();
	}else if(document.querySelector("#passwd").value!=document.querySelector("#passwd2").value){
		alert("비밀번호가 일치하지 않습니다.")
		event.preventDefault();
	}else if(document.querySelector("#username").value.length==0){
		alert("이름 필수")
		event.preventDefault();
	}else if(document.querySelector("#sample4_postcode").value.length==0){
		alert("우편번호 찾기를 통해 주소를 입력해주세요.")
		event.preventDefault();
	}else if(document.querySelector("#phone2").value.length==0 ||
			document.querySelector("#phone3").value.length==0){
		alert("전화번호 필수")
		event.preventDefault();
	}else if(document.querySelector("#email1").value.length==0 ||
			document.querySelector("#email2").value.length==0){
		alert("이메일 필수")
		event.preventDefault();
	}
...
function ch() {
	var mail = document.querySelector("#emailSelect").value;
	document.querySelector("#email2").value = mail;
	
}
</script>
<form action="memberAdd" method="post" onsubmit="validcheck()">
...
<input type="text" name="addr1" id="sample4_roadAddress" placeholder="도로명주소" readonly>
...
이메일:<input type="text" name="email1" id="email1">@
       ...
       <select  id="emailSelect" onchange="ch()">
